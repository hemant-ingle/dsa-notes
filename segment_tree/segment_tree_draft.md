...
[DONE] 1. Sum/Min/Max Range queries [Array modification allowed] - Build: O(n), Update: O(logn), Query: O(logn)
[DONE] 2. Max/Min and Number of times it appears
struct Node  {
    int element;
    int frequency;
}
[DONE] 3. Compute GCD/LCM
4. Counting the number of zeros, searching for the k-th zero
5. Search for an array prefix with a given amount
6. Search for the first element greater than a given amount
...
1. Simple 2D Segment Tree
2. Compression of 2D Segment Tree

Preserving the history of its values (Persistent Segment Tree)
1. Finding the k-th smallest number in a range

Dynamic segment tree (Also known as implicit segment tree or sparse segment tree)
